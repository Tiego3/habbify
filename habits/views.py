...
def toggle_progress(request, pk):
    """Toggle today's completion for a given habit."""
    habit = get_object_or_404(Habit, pk=pk, user=request.user)
    today = date.today()
    p, created = Progress.objects.get_or_create(habit=habit, date=today, defaults={'completed': True})
    if not created:
        # toggle
    ...
    if request.is_ajax() or request.headers.get('x-requested-with') == 'XMLHttpRequest':
        return JsonResponse({'completed': p.completed, 'streak': habit.current_streak(), 'count': habit.completion_count()})
...
def habit_chart_data(request, pk):
    habit = get_object_or_404(Habit, pk=pk, user=request.user)
    # Prepare data for Chart.js: last 7 days completion rate
    today = date.today()

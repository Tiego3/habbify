...
class Habit(models.Model):
    ...
    def is_completed_today(self):
        return self.logs.filter(log_date=date.today(), is_completed=True).exists()

    def last_14_days(self):
        """Returns progress for last 14 days (for daily habits)."""
        today = timezone.now().date()
        start = today - timedelta(days=13)
        completed_dates = set(
            self.logs.filter(log_date__range=[start, today], is_completed=True)
        ...
        for i in range(13, -1, -1):
            ...
            days.append({
                'date': date_check,
                'completed': date_check in completed_dates,
                'is_today': date_check == today,
            })
        return days

    def last_7_days(self):
        """Returns progress for last 7 days (legacy helper)."""
        return self.last_14_days()[-7:]
